...
patients_procedure <- read_csv("~/mimic/hosp/procedures_icd.csv.gz",
                               show_col_types = F) |>
  filter(subject_id == patient_id) |>
  # Make chartdate as POSIXct in order to draw the plot
...
patients_procedure <- left_join(patients_procedure, procedure,
                                by = c("icd_code", "icd_version")) |>
  # long_title is not good to display, change it to a shorter one
...
patients_adt <- read_csv("~/mimic/hosp/transfers.csv.gz",
  ...
  select(-transfer_id) |>
  arrange(intime) |>
  # both ICU and CCU have the letters "CU", so we can use it to detect
...
# Build a blank data to set the range of the x-axis
...
ggplot(empty_data, mapping = aes(x = CalenderTime, y = Information)) +
  ...
  geom_point(data = patients_procedure,
             mapping = aes(x = chartdate,
                           ...
                           shape = Procedure),
             size = 3) +
  geom_segment(data = patients_adt,
...
print(admission_tble, width = Inf)
```
...
```
Notice that there are more female patients than male patients. The age of the patients has two peaks, one is around 60 years old, and the other is around 20 years old. According to the violin graph, the distribution of age is quite similar for male and female patients. However, there are more female patients in the age group of 20-40 years old, and above 80 years old.
...
# Get the name of the lab items
items_id <- c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)
lab_items <- read_csv("~/mimic/hosp/d_labitems.csv.gz",
                      show_col_types = FALSE) |>
  filter(itemid %in% items_id) |>
  select(itemid, label)
lab_items <- setNames(lab_items$label, lab_items$itemid)


# icu_id is the subject_id of patients went to ICU
...
labevents_tble <- arrow::open_dataset("labevents_pq", format = 'parquet') |>
  ...
  # filter the labevents we are interested in
  filter(itemid %in% items_id) |>
  
  # only keep the labevents of patients went to ICU
  filter(subject_id %in% icu_id$subject_id) |>
  collect() |>
  
  # Two dataset shares the same subject_id, stay_id, and intime
  left_join(icustays_tble[,c("subject_id", "stay_id", "intime")],
            by = "subject_id") |>
  
  # only keep the labevents before the ICU stay
  filter(storetime < intime) |>
  group_by(stay_id, itemid) |>
  
  # only keep the last labevents before the ICU stay
  ...
  arrange(subject_id) |>
  rename_with(~lab_items[.x], .cols = -c(subject_id, stay_id)) |>
  select(subject_id, stay_id, Bicarbonate, Chloride, Creatinine,
         Glucose, Potassium, Sodium, Hematocrit, `White Blood Cells`)

rm(lab_items, items_id)
print(labevents_tble, width = Inf)
```

Notice that For patient 10171405, stay_id 30517917, the patient's lab result has two different result at the same storetime. So I include the second result in the final table.
...
# Get the item_id of the vitals
vitals_id <- c(220045, 220179, 220180, 223761, 220210)

# Get the name of the vitals
vitals <- read_csv("~/mimic/icu/d_items.csv.gz",
                   show_col_types = FALSE) |>
  filter(itemid %in% vitals_id) |>
  select(itemid, abbreviation)

vitals <- setNames(vitals$abbreviation, vitals$itemid)


chartevents_tble <- arrow::open_dataset("chartevents_pq", format = 'parquet') |>
  select(subject_id, itemid, charttime, valuenum) |>
  
  # filter the chartevents we are interested in
  filter(itemid %in% vitals_id) |>
  collect() |>
  
  # two dataset shares the same subject_id, stay_id, intime, and outtime
  left_join(icustays_tble[,c("subject_id", "stay_id", "intime", "outtime")],
            by = "subject_id") |>
  
  # only keep the chartevents during the ICU stay
  filter(charttime >= intime & charttime <= outtime) |>
  group_by(stay_id, itemid) |>
  
  # only keep the first chartevents during the ICU stay
  ...
  pivot_wider(names_from = itemid, values_from = valuenum) |>
  arrange(subject_id) |>
  rename_with(~vitals[.x], .cols = -c(subject_id, stay_id, outtime)) |>
  select(subject_id, stay_id, HR, NBPs, NBPd, RR, `Temperature F`)
...
# Give different colors to different race group
race_los$racegp <- "Others"
race_los <- race_los |>
  mutate(racegp = case_when(
...
nameorder <- race_los$race[order(race_los$racegp, race_los$mean_los)]
race_los$race <- factor(race_los$race, levels = nameorder)
race_los |>
  ggplot(aes(x = mean_los, y = race)) +
  geom_segment(aes(yend = race), xend = 0, color = "grey50") +
  geom_point(size = 2, aes(color = racegp)) +
  theme_bw() +
...
plot_7 <- ggplot(mimic_icu_cohort, aes(x = `White Blood Cells`, y = los)) +
  geom_point(alpha = 0.05) +
  labs(x = "WBC", y = "LOS")
...
plot_1 <- ggplot(mimic_icu_cohort, aes(x = HR, y = los)) +
  geom_point(alpha = 0.05) +
...
plot_4 <- ggplot(mimic_icu_cohort, aes(x = `Temperature F`, y = los)) +
  geom_point(alpha = 0.05) +
