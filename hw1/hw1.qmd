...
zcat < ~/mimic/hosp/patients.csv.gz \
    ...
    | wc -l 
```
**Answer: ** The first few lines of `admissions.csv.gz` are displayed above. There are 431232 rows in this data file (including one row for the name of the variables). There are 180733 unique patients in this data file. They do not match the number of patients listed in the `patients.csv.gz` file, which is 299712.
