...
```
There are 431232 lines in `admissions.csv.gz`, 118171368 lines in `labevents.csv.gz`, and 299713 lines in `patients.csv.gz`, including the header line. Since there are too many lines in `labevents.csv.gz`, so I set `eval: false` for the above code.
...
zcat < ~/mimic/hosp/patients.csv.gz \
    ...
    | wc -l 
```
The first few lines of `admissions.csv.gz` are displayed above. There are 431232 rows in this data file (including one row for the name of the variables). There are 180733 unique patients in this data file (excluding the header line). They do not match the number of patients listed in the `patients.csv.gz` file, which is 299712 (excluding the header line).
...
```
Notice that for labevents.csv.gz, it takes 1 min 23 sec to run the `~/mimic/hosp/labevents.csv.gz | wc -l` command, while for the decompressed file labevents.csv, it only takes 15 sec to run the `wc -l labevents.csv` command. Compressed files save disk space, which is beneficial when dealing with large datasets. However, if we read from a compressed file (zcat), it will introduce additional processing time comparing to directly reading csv files.
