...
best_gb
```
According to the result, when `tree_depth = 5`, `learn_rate = 0.0316`, the model has the best performance via 5-fold CV. It has mean AUC of 0.639. In order to shorten the training time, and to avoid overfitting, I set the `mtry = 9` (randomly choose 9 variables from predictors).
...
```
According to the result, when `cost = 0.707`, `rbf_sigma = 0.0316`, the model has the best performance via 2-fold CV. It has mean AUC of 0.620. Since the training time of SVM is very long, I only use 2-fold CV to tune the model. Using 2-fold CV and 3 by 3 grid search, the training process still takes more than 15 mins.
...
best_logit
```
According to the result, when `penalty = 1e-05`, `mixture = 0.25`, the model has the best performance via 5-fold CV. It has mean AUC of 0.603. There are not obvious "U curve" in the plot of grid search, and many combinations of penalty and mixture have similar performance. I simply choose the best combination on the top of the list.
...
```
The stacking model combines 3 XGBoost models and 2 SVM models. The Boosting model has the highest weight, followed by the SVM model. The logistic model is not included in the final stacking model.
