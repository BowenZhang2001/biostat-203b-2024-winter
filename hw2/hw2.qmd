...
The first 10 rows of the result tibble match those in Q2.3. However, `charttime` is 7 hours earlier than that in Q2.3. This is because the `charttime` in the result tibble is in local time zone, while the `charttime` in Q2.3 is considered as UTC time zone by default. The code below shows that is we turn the `charttime` to UTC time zone, the first 10 rows of the result tibble match those in Q2.3.
