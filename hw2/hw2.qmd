...
admissions |> head(width = Inf)
```
After changing some character columns to factors, the memory usage of the result tibble is 41 MB, which is the less than the default data types. The run time is 1.5 seconds, which is also shorter than the default data types.
