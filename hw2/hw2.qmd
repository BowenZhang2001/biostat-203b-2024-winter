...
It takes more than 5 minutes on my laptop, and it crashed after that. The following is the screenshot of the error message. When trying to ingest the data, the memory usage of RStudio increases rapidly, and is over 7GB in the red circle in the screenshot. Then the RStudio crashes. ![crash](./crash.jpg)
...
# Taking too much time, set eval to false
...
lab_arrow |> collect() |> arrange(subject_id) |> head(10)

```

Ingest+select+filter process takes 0.08 seconds. The number of rows matches those in Q2.3.

However, the first 10 rows of the result tibble do not match those in Q2.3. If we arrange the result tibble by `subject_id` in ascending order, `charttime` is 7 hours earlier than that in Q2.3. This is because the `charttime` in the result tibble is in local time zone, while the `charttime` in Q2.3 is considered as UTC time zone by default. The code below shows that is we turn the `charttime` to UTC time zone, the first 10 rows of the result tibble match those in Q2.3.

```{r}
#| eval: true

lab_arrow |> collect() %>% .$charttime |> as.POSIXct(tz = "UTC") |> head(10)
rm(lab_arrow)
```
